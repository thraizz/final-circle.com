...
echo "===== Last Circle Backend Update ====="
...
ssh $REMOTE_USER@$REMOTE_HOST "cd $REMOTE_DIR && \
  ...
  if command -v systemctl >/dev/null 2>&1 && sudo -n systemctl is-active lastcircle.service >/dev/null 2>&1; then
  ...
  elif command -v systemctl >/dev/null 2>&1 && systemctl --user is-active lastcircle.service >/dev/null 2>&1; then
  ...
  # Fall back to direct process restart
    ...
    pkill -f last-circle-server 2>/dev/null || true && \
    PORT=$APP_PORT nohup ./last-circle-server > lastcircle.log 2>&1 & \
    echo 'Server restarted in background.'
  fi"

echo "===== Update completed! ====="
echo "The Last Circle backend has been updated and restarted on $REMOTE_HOST:$APP_PORT"
echo ""
